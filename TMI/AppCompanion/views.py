...
def search(request):
    ...
    if q_start_date: 
        q_start_date = datetime.datetime.strptime(q_start_date, '%m/%d/%Y')
        qs = qs.filter(end_date__gte = q_start_date)
        
        if q_end_date:
            q_end_date = datetime.datetime.strptime(q_end_date, '%m/%d/%Y')
            qs = qs.filter(start_date__lte = q_end_date)
    else:
        if q_end_date:
            q_end_date = datetime.datetime.strptime(q_end_date, '%m/%d/%Y')
            qs = qs.filter(start_date__gte = q_end_date)       
